(function () {
    ...
    document.getElementById('chatStartBtn').addEventListener('click', async () => {
        ...
        try {
        // Start session
        const startSessionRes = await fetch('http://localhost:8001/api/v1/bots/sessions/start-text', {
            method: 'POST',
            headers: {
        ...
        chat.connect = {
            ...
            try {
                ...
                `http://localhost:8001/api/v1/bots/sessions/${sessionId}/send-message?message=${encodedMsg}&use_rag=true`,
                {
                    method: 'POST',
                    headers: {
                    'Authorization': `Bearer ${token}`,
