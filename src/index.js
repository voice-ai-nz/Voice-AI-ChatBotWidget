(function () {
    ...
    script.onload = async () => {
        ...
        const primaryColor =
        hostScript?.getAttribute("data-primary-color") || "#007bff"; // default blue
        const headerTextColor =
        hostScript?.getAttribute("data-header-text-color") || "white"; // default white
        const buttonTextColor =
        hostScript?.getAttribute("data-button-text-color") || "white"; // default white
        const apiBaseUrl =
        hostScript?.getAttribute("data-api-url")?.replace(/\/+$/, "") ||
        "https://app.voice-ai.co.nz/api/v1"; // default
        const logoUrl = hostScript?.getAttribute("data-logo-url") || null;
        ...
        // Header HTML (with optional logo)
        const headerContent = logoUrl
        ? `<img src="${logoUrl}" alt="Chat Logo" style="height:24px;margin-right:10px;vertical-align:middle;">`
        : "";
        ...
        container.innerHTML = `
            <div id="chatForm" style="height:100%;display:flex;flex-direction:column;">
                ...
                align-items:center;
                ">
                ${headerContent} <span>Chat with Us</span>
                </div>
                ...
                <button id="chatStartBtn" style="padding:10px;width:100%;background:${primaryColor};color:${buttonTextColor};border:none;border-radius:5px;cursor:pointer;">Start Chat</button>
                </div>
            </div>
            `;
